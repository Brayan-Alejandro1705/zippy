...
const VendorEditarProductoPage = () => {
  const { id }        = useParams();
    ...
    productosService.obtener(id)
      .then(({ data }) => {
        ...
        setForm({
          ...
          disponible:  data.es_visible ?? true,
        });
        if (data.imagenes?.[0]) setPreview(data.imagenes[0]);
      })
      .catch(() => { if (activo) setNotFound(true); })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await productosService.actualizar(id, {
        nombre:      form.nombre,
      ...
      addToast(`"${form.nombre}" actualizado correctamente`, 'success');
      navigate('/vendor/productos');
    } catch (err) {
      const msg = err.response?.data?.detail || 'No se pudo actualizar el producto. Intenta de nuevo.';
      addToast(msg, 'error');
    } finally {
      setSaving(false);
    }
            ...
            {step === 1 ? (
              ...
              <>
                ...
                <div className="vnp-field vnp-toggle-row">
                  <div>
                    <span className="vnp-toggle-label">Producto disponible</span>
                    <p className="vnp-toggle-sub">Si está activo, los clientes pueden verlo y comprarlo</p>
                  </div>
                  <button
                    type="button"
...
export default VendorEditarProductoPage;